...
import compilerDesign.hw5.grammar.Message;
 ...
 */
public class ProjectFinalGui extends JFrame implements ActionListener
{
	/**
	 * 
	 */
	...
	public ProjectFinalGui()
	{
		this(null);
	}
	
	public ProjectFinalGui(String grammarPath)
	{
		super(APP_TITLE);
	...
	private void createStyles()
	{
		// add styles
		//statusText
		Style s = statusText.addStyle("Blue", null);
		StyleConstants.setForeground(s, Color.BLUE);
		
		s = statusText.addStyle("Red", null);
		StyleConstants.setForeground(s, Color.RED);
		
		s = statusText.addStyle("Green", null);
		StyleConstants.setForeground(s, new Color(0, 128, 0));
		
		//mooStatusText
		s = mooStatusText.addStyle("Blue", null);
		StyleConstants.setForeground(s, Color.BLUE);
		
		s = mooStatusText.addStyle("Red", null);
		StyleConstants.setForeground(s, Color.RED);
		
		s = mooStatusText.addStyle("Green", null);
		StyleConstants.setForeground(s, new Color(0, 128, 0));
	}
	...
	private void doCompile()
	{
		try
		{
			...
			if(compileAttempted)
				Grammar5.ReInit(srcReader);
			else
				new Grammar5(srcReader);
			...
			printOutput(Grammar5.getOutput());
		}
		catch (Throwable e)
		{
			printOutput(Grammar5.getOutput(), e);
		}
	}
	...
	{
		StyledDocument statusDoc = statusText.getStyledDocument();
		StyledDocument symbolsDoc = mooStatusText.getStyledDocument();
		...
		if(e != null)
		{
			//e.printStackTrace(); //XXX DBEUG
			
			startPos = statusDoc.getLength();
			
			appendln("Error encountered during compilation.");
			appendln();
			
			len = statusDoc.getLength() - startPos;
			
			statusDoc.setCharacterAttributes(
					startPos, len,
					statusText.getStyle("Red"), true);
		}
		...
		for(Message m : output)
		{
			...
			// so display if if the user wants to see it
			else if(isVerbose())
			{
				startPos = statusDoc.getLength();
				...
				statusDoc.setCharacterAttributes(
						startPos, len,
						statusText.getStyle("Blue"), true);
			}
		}
		
		if(e == null)
		{
			statusDoc = statusText.getStyledDocument();
			...
			statusDoc.setCharacterAttributes(
					startPos, len,
					statusText.getStyle("Green"), true);
			
			if(isMooMode())
			{
				appendln("The great Wiki says that..." , mooStatusText);
				...
				for(int i = 0; i < 1000; i++)
				{
					...
					startPos = symbolsDoc.getLength();
					
					appendln("ERROR: right hand recursion.", mooStatusText);
					
					len =  symbolsDoc.getLength() - startPos;
					
					// color the moo mode text
					symbolsDoc.setCharacterAttributes(
						startPos, len,
						mooStatusText.getStyle("Red"), true);
			...
			appendln(Grammar5.getSymbolTableText(), mooStatusText);
			
			len = symbolsDoc.getLength() - startPos;
			
			symbolsDoc.setCharacterAttributes(
					startPos, len,
					mooStatusText.getStyle("Green"), true);
			
			// emit code !!!
			try
			{
				File outFile = new File("Jasmin_out.j");
				
				Grammar5.emitJasmin(outFile.getAbsolutePath());
				
				startPos = symbolsDoc.getLength();
				
				appendln("Jasmin code generated:", mooStatusText);
				appendln(mooStatusText);
				appendln(outFile.getAbsolutePath(), mooStatusText);
				
				len = symbolsDoc.getLength() - startPos;
				
				symbolsDoc.setCharacterAttributes(
						startPos, len,
						mooStatusText.getStyle("Blue"), true);
			}
			// uh oh...
			// something happened with the jasmin writing to disk thing
			catch (IOException e1)
			{
				startPos = symbolsDoc.getLength();
				
				appendln("Error generating Jasmin code:", mooStatusText);
				appendln(mooStatusText);
				appendln(e1.toString(), mooStatusText);
	...
	{
		append(Grammar5.NEWLINE, target);
	}
