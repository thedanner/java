...
class TreeIterator<AnyType> implements Iterator<AnyType> {
	private TreeNode<AnyType> root, last;
	private Stack<TreeNode<AnyType>> stack;
	
	/**
	 * Constructor for creating an Iterator for the (sub)tree rooted at tn
	 * @param  tn  a TreeNode
	 */
	public TreeIterator (TreeNode<AnyType> tn) {
		root = tn;
		last = null;
		stack = new Stack<TreeNode<AnyType>>( );
		initStack(root);
	}
	 ...
	 */
	private void initStack(TreeNode<AnyType> node) {
		if (node != null) {
			stack.push(node);
